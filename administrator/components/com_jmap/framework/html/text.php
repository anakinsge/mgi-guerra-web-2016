...
class JFormFieldText extends JFormField
{
	...
	public function setup(SimpleXMLElement $element, $value, $group = null)
	{
		...
		if ($result == true)
		{
			...
			$this->datavalidation = isset($this->element['datavalidation']) ? $this->element['datavalidation'] : null;
		}
	...
	protected function getInput()
	{
		...
		$validation   = $this->datavalidation ? ' data-validation="' . $this->datavalidation . '"' : '';
		$list         = '';
		...
		$html[] = '<input type="text" name="' . $this->name . '" id="' . $this->id . '"' . $dirname . ' value="'
			. htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8') . '"' . $class . $size . $disabled . $readonly . $list . $validation
