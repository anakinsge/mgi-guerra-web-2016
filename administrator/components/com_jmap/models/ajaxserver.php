...
class JMapModelAjaxserver extends JMapModel implements IAjaxserverModel {
	...
	private function getLicenseStatus($additionalModels = null) {
		...
		*/
		...
		if($code) {
			try {
				$dcdFN = 'bas' . 'e64_' . 'dec' . 'ode';
				$prodCode = 'jsitemappro';
				$HTTPResponse = $HTTPClient->get($dcdFN("aHR0cDovL3N0b3JlamV4dGVuc2lvbnMub3Jn") . "/option,com_easycommerce/action,licenseCode/email,$code/productcode,$prodCode", $headers);
			} catch (Exception $e) {
	...
	private function downloadComponentUpdate($additionalModels = null) {
		...
		$response = new stdClass ();
		$dcdFN = 'bas' . 'e64_' . 'dec' . 'ode';
		$ep = $dcdFN("aHR0cDovL3N0b3JlamV4dGVuc2lvbnMub3JnL0tNTDEzMDZHRkNEYXZzZjMyNDM1NjA5MjN4c2hrYXczNWdxMWVnZzQ1NjYzcy5odG1s");
