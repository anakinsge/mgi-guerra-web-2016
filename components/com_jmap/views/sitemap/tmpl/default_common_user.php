...
if(array_key_exists($targetOption, $supportedRouterHelperAdapters)) {
	...
	if(	$targetOption == 'com_easyblog' &&
		!file_exists(JPATH_SITE . '/components/'.$targetOption.'/'.$folderPath.'/'.$filePath.'.php') && // Not the frontend router as a standard
		 file_exists(JPATH_ADMINISTRATOR . '/components/'.$targetOption.'/includes/'.$filePath.'.php')) { // But a proprietary admin router
